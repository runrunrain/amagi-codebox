...
Wails auto-generates TypeScript bindings under `frontend/wailsjs/go/<pkg>/` from the bound Go methods whenever you run `wails dev`/`wails build`. **Never hand-edit `frontend/wailsjs/`** — regenerate it. **Binding generation silently no-ops when the `wails` CLI version differs from the vendored `github.com/wailsapp/wails/v2` in `go.mod`** (it prints "Generating bindings: Done" but writes nothing / omits new methods). Use a CLI matching the vendored version, e.g. `go install github.com/wailsapp/wails/v2/cmd/wails@v2.11.0`, then `wails build`. Confirm new bound methods actually appear in `App.js`/`App.d.ts` and new return types in `models.ts` before committing. `frontend/src/api/*.ts` modules wrap those raw bindings into typed, ergonomic functions (one per domain: `provider.ts`, `session.ts`, `plugin.ts`, etc.), and Pinia stores in `frontend/src/stores/` consume them. Flow: Vue view → composable/store → `src/api/*.ts` → `wailsjs/go/...` → Go service.
