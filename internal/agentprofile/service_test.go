...
// 非法 JSON 拒绝、omp 缺失时的降级、delete 与名字校验。
// 全部走临时目录（HOME + PI_CODING_AGENT_DIR 重定向），不碰真实文件。
...
const companyOmp = `{"profile":"omp-company"}`
...
func TestCaptureApplyRoundTrip(t *testing.T) {
	...
	ompPath := filepath.Join(home, ".omp", "agent", "amagi.json")
	...
	// 预览：单档 JSON 含两侧内容（内嵌 JSON 被转义，解析后比较）
	...
	writeFixture(t, ompPath, `{"profile":"omp-edited"}`)
...
func TestApplyCreatesBackup(t *testing.T) {
	...
	writeFixture(t, piPath, homePi)
	writeFixture(t, ompPath, `{"profile":"omp-v2"}`)
	if err := s.ApplyAgentProfile("company"); err != nil {
		t.Fatalf("ApplyAgentProfile #1: %v", err)
	}
	assertSingleBackup(t, piPath, homePi)
	assertSingleBackup(t, ompPath, `{"profile":"omp-v2"}`)
...
// TestInvalidJSONRejected 覆盖：Save 拒绝非 JSON 内容；存储被手工污染后
// Apply 同样拒绝且不动 live 文件。
func TestInvalidJSONRejected(t *testing.T) {
	...
	if err := s.SaveAgentProfile("bad", "", "}also bad{"); err == nil {
		t.Fatal("SaveAgentProfile should reject invalid omp JSON")
	}
...
func TestOmpMissingCaptureAndApply(t *testing.T) {
	...
	writeFixture(t, filepath.Join(home, ".omp", "agent", "keep.json"), "{}")
	if err := s.ApplyAgentProfile("company"); err != nil {
		t.Fatalf("ApplyAgentProfile with empty omp content: %v", err)
	}
	if _, err := os.Stat(filepath.Join(home, ".omp", "agent", "amagi.json")); !os.IsNotExist(err) {
		t.Fatalf("omp amagi.json must stay absent when content empty, stat err=%v", err)
	}
}
...
func TestEnvAgentDirOverride(t *testing.T) {
	...
	// 该目录下的 amagi.json 同时是 pi 与 omp 的 live 文件（现有语义）
	...
	if p.Pi != companyPi || p.Omp != companyPi {
...
func TestSaveAgentProfileExplicitContent(t *testing.T) {
	...
	ompData, err := os.ReadFile(filepath.Join(home, ".omp", "agent", "amagi.json"))
	if err != nil {
