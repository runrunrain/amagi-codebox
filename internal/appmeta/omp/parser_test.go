...
// TestExtractUsageRecordsOmpSample mirrors omp's session JSONL (same shape as
// pi docs/session-format.md): a session header, a user message, two assistant
// turns (one carrying cost), and noise lines (toolResult without usage,
// model_change) that must be ignored.
func TestExtractUsageRecordsOmpSample(t *testing.T) {
	dir := t.TempDir()
	...
	if r1.InputTokens != 1200 || r1.OutputTokens != 80 || r1.CacheReadInputTokens != 300 {
		t.Errorf("tokens1 = %d/%d/%d, want 1200/80/300", r1.InputTokens, r1.OutputTokens, r1.CacheReadInputTokens)
	}
	if !r1.CostProvided {
	...
	// omp native cost is USD — must be tagged explicitly.
	if r1.CurrencyCode != "USD" {
		t.Errorf("currencyCode1 = %q, want USD (omp native cost)", r1.CurrencyCode)
	}
	...
	if r1.DedupKey == "" || r1.DedupKey[:4] != "omp:" {
		t.Errorf("dedup1 = %q, want omp: prefix", r1.DedupKey)
	}
	...
	if r2.CostProvided || r2.NativeCost != 0 || r2.CurrencyCode != "" {
		t.Errorf("cost2 = provided:%v native:%d currency:%q, want false/0/empty", r2.CostProvided, r2.NativeCost, r2.CurrencyCode)
	}
...
// TestExtractUsageRecordsOmpIncrementalSessionContext verifies an incremental
// resume (startOffset>0) still resolves SessionID and ProjectDir from the
// header instead of degrading to file-basename / encoded-dir fallbacks.
func TestExtractUsageRecordsOmpIncrementalSessionContext(t *testing.T) {
	dir := t.TempDir()
...
// TestExtractUsageRecordsOmpNonAssistantUsage verifies toolResult, compaction
// and branch_summary usages are all counted, each with a stable dedup key and
// USD native cost.
func TestExtractUsageRecordsOmpNonAssistantUsage(t *testing.T) {
	dir := t.TempDir()
	...
	if branch.InputTokens != 80 || branch.OutputTokens != 8 {
		t.Errorf("branch_summary tokens: in=%d out=%d, want 80/8", branch.InputTokens, branch.OutputTokens)
	}
...
// TestExtractUsageRecordsOmpForkNoDoubleCount verifies history copied by
// omp's fork/clone into a new file dedups against the original, while
// genuinely-new entries in the fork are still counted.
func TestExtractUsageRecordsOmpForkNoDoubleCount(t *testing.T) {
	dir := t.TempDir()
	...
	// fork: header points back at the original, then COPIES e1 verbatim (same
	// entry ids) and adds a brand-new turn e2.
	...
	// The copied e1 in the fork must share the ORIGINAL's dedup key.
	copied := forkRecs[0]
...
// TestExtractUsageRecordsOmpDedupCollisionSafe verifies that the SAME 8-hex
// entry id in two UNRELATED sessions (no parentSession link) yields distinct
// dedup keys — i.e. dedup is content-scoped, not entry-id-global.
func TestExtractUsageRecordsOmpDedupCollisionSafe(t *testing.T) {
	dir := t.TempDir()
	...
	// together even when no ancestor file is resolvable.
...
// TestExtractUsageRecordsOmpTruncatedTail verifies that an incomplete trailing
// line (no newline) is NOT committed: its offset is preserved so the completed
// line is picked up on the next sync instead of being skipped.
func TestExtractUsageRecordsOmpTruncatedTail(t *testing.T) {
	dir := t.TempDir()
...
// TestExtractUsageRecordsOmpBadJSONContinues verifies a malformed line is skipped.
func TestExtractUsageRecordsOmpBadJSONContinues(t *testing.T) {
	dir := t.TempDir()
