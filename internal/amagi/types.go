...
type AmagiSettings struct {
	Model                    string                             `json:"model"`                                // Current active ModelPreset group name
	...
	ModelPresets             map[string]ModelPresetGroup        `json:"model_presets"`                        // Preset group configurations
	AlwaysThinkingEnabled    *bool                              `json:"always_thinking_enabled,omitempty"`    // Always enable thinking
	EffortLevel              string                             `json:"effort_level,omitempty"`               // low/medium/high/xhigh/max
...
type AmagiModelPreset struct {
	...
	EffortLevel     string                 `json:"effort_level,omitempty"` // Per-preset effort: low/medium/high/xhigh/max
...
// ModelPresetGroup represents a named collection of model presets.
// Dashboard selects at the group level; each group contains multiple sub-presets.
type ModelPresetGroup struct {
	Description   string                      `json:"description,omitempty"`
	DefaultPreset string                      `json:"default_preset,omitempty"` // Key into Presets map
	Presets       map[string]AmagiModelPreset `json:"presets"`
}
...
func DefaultAmagiSettings() *AmagiSettings {
	return &AmagiSettings{
		...
		ModelPresets:             map[string]ModelPresetGroup{},
