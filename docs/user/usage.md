...
集中展示 Claude Code、Codex 与 OpenCode 会话日志中的模型用量和成本。页面默认选择“近 30 天”和“仅会话日志”，避免把代理实时记录与会话日志重复计算。

- **时间范围**：可一键切换今日、近 7 天、近 30 天、本月，或选择自定义的起止日期。
- **Token 用量**：以 `m`（百万 Token）展示新输入、输出、缓存读取和缓存写入，固定保留两位小数；对 Codex 的缓存读取会先从原始输入中拆出，避免重复统计。
- **缓存经济性**：模型明细展示缓存命中率、缓存折算 Token、缓存命中成本及相对全价输入节省的成本。缓存命中相关金额按价格表中的缓存命中单价计算。
- **趋势**：成本与 Token 使用独立切换；每条曲线对应一个模型/供应商组合，可选择单个模型查看，避免将不同模型混为总曲线。
- **价格表**：支持查看、编辑、重置模型的输入、输出、缓存读取与缓存写入单价。保存价格后会重算本地估算记录；OpenCode 已提供的原始账单金额会保留。
