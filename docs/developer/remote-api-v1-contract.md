...
`WebUIStatus` = `{state, url?}`，`state` 为 5 值闭合枚举（`KnownWebUIPlaneStates`）：`probing` / `available` / `unavailable` / `ended` / `unknown`（镜像宿主侧 `internal/webui.State`；未注册/非 pi 会话 → `unknown`，端点仍 200，状态机语义优先于 404）。`url` 为条件字段：仅 `available` 时必填且必为同源相对代理路径 `/webui/{sessionId}/[/#/t=<capability-token>]` —— **token 只经 URL fragment 承载**，绝不进请求行/query/日志/错误体；仅下发给已通过 device Cookie 鉴权的设备。服务端每请求执行一轮探测（与桌面端轮询同一状态机推进路径）。
