...
- `GET /sessions` 列表语义（与桌面端一致，2026-10）：仅返回「打开未关闭」的会话（lifecycle 态 running/stopping；stopped/exited/unavailable 的关闭会话不再出现在列表——关闭即从大厅消失，残留即 bug）；排序与桌面端会话列表同一规则：`startedAt` 降序（按时刻 time 比较，非 RFC3339 字符串——字符串比较在 DST 偏移变化时会错序），`sessionId` 升序作并列打破——`startedAt` 不可变，跨轮询稳定，不随活动时间重排。`GET /sessions/{id}` 详情端点不变：关闭会话在被移除（tombstone）前仍可单查。
