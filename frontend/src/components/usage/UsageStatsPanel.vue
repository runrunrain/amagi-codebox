<template>
  <div class="stats-panel">
    ...
    <EmptyState
      v-else-if="summary && summary.totalRequests === 0"
      title="暂无使用数据"
...
/**
 * UsageStatsPanel — 使用统计子页（/usage/stats）。
 *
 * P2 分页化迁移：原 UsageView.vue 的统计内容整体搬运（设计 §5「纯搬运
 * 不改逻辑」），PageHead / RemoteScopeBanner / 页切换 Segmented 上移到
 * UsageView 壳层；PageHead actions 里的「立即同步」按钮也随壳层走
 * （EmptyState 内的同步按钮保留在本面板）。
 */
...
<style scoped>
/* 原壳层 .view-usage 的列间距迁移为面板自身布局 */
.stats-panel { display: flex; flex-direction: column; gap: 22px; }
.range-row, .range-presets, .trend-tabs, .trend-toolbar { display: flex; align-items: center; gap: 10px; }
.summary-sub, .card-sub, .trend-hint, .metric-sub, .range-detail { color: var(--tertiary); font-size: 12px; }
